...
void main() {
    ...
    vec3 col = BASE;
    col += MIST_FAR * pow(max(f1, 0.0), 2.8) * 0.40;
    col += MIST_MID * pow(max(f2, 0.0), 2.6) * 0.60;
    ...
    float wisp = pow(max(f3, 0.0), 5.0);
    float veil = wisp * 1.1;
    veil = veil > 0.5 ? 0.5 : veil;
    col = col * (1.0 - veil * 0.45) + MIST_NEAR * veil * 0.30;
