...
void main() {
    ...
    // Premultiplied alpha: dense fog opaque, gaps reveal the stars below.
    float alpha = cover * 0.75 + pow(max(fog, 0.0), 1.8) * 0.35 + veil * 0.4;
    alpha = alpha > 0.96 ? 0.96 : (alpha < 0.05 ? 0.05 : alpha);
