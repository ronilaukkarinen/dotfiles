...
void main() {
    ...
    // Quantum motes: tiny drifting embers of light.
    vec2 mc = (screenPx + u_camera * 0.85) / 220.0 + vec2(t * 0.014, t * 0.010);
    vec2 mcell = floor(mc);
    for (int my = -1; my <= 1; my++) {
        for (int mx = -1; mx <= 1; mx++) {
            vec2 m = mcell + vec2(float(mx), float(my));
            if (hash(m + 111.1) > 0.85) {
                vec2 mpos = m + 0.2 + 0.6 * vec2(hash(m + 117.7), hash(m + 123.3));
                float md = length(mc - mpos);
                float mglow = exp(-md * md * 60.0);
                float mb = 0.6 + 0.4 * sin(t * 0.7 + hash(m + 131.3) * 6.2831);
                col += vec3(0.55, 0.50, 0.85) * mglow * mb * 0.30;
            }
        }
    }
    ...
    gap = gap < 0.52 ? 0.0 : (gap - 0.52) / 0.48;
    float alpha = 0.93 - gap * 0.80 + veil * 0.2;
