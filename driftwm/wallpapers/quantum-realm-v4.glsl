...
const vec3 BASE = vec3(0.014, 0.012, 0.030);   // near-black violet
const vec3 MIST_FAR  = vec3(0.26, 0.28, 0.44);  // cool, dim, distant
const vec3 MIST_MID  = vec3(0.50, 0.44, 0.70);  // the purple body
const vec3 MIST_NEAR = vec3(0.66, 0.58, 0.86);  // pale near-wisps
...
void main() {
    ...
    vec3 col = BASE;
    col += MIST_FAR * pow(max(f1, 0.0), 3.0) * 0.34;
    col += MIST_MID * pow(max(f2, 0.0), 3.0) * 0.50;
    ...
    col += tint * (0.18 + 0.82 * pow(max(fog, 0.0), 2.0)) * 0.80;

    // Near plane: fine wisps VEIL what is behind them: true occlusion depth.
    float wisp = pow(max(f3, 0.0), 3.4);
    float veil = wisp * 1.3;
    veil = veil > 0.8 ? 0.8 : veil;
    col = col * (1.0 - veil * 0.55) + MIST_NEAR * veil * 0.42;
